...
public class PedidosBDD {
	...
	public ArrayList<Pedidos> consultarPedidoProveedor(String idProveedorConsulta) throws KrakeDevException {
		
		ArrayList<Pedidos> pedidos = new ArrayList<Pedidos>();
		Connection con = null;
		PreparedStatement psBuscarProveedor= null; // Valido proveedor 
	    PreparedStatement psCabeceras = null; // Valido que existe el pedido
	    PreparedStatement psDetalles = null; // Busco detalles del pedido
	    
	    ResultSet rsProveedor = null; // id del proveedor
	    ResultSet rsCabeceras = null; // id del pedido
	    ResultSet rsDetalles = null; // detalle del pedido
		
		
		// Idea general: Mostrar el pedido y su detalle por el id del proveedor
		// Se relacionan varias tablas.
	    // Valido id que exista, obtengo datos 
	    // Valido los ids de los pedido con el id del proveedor si existe
	    // Obtengo los detalles con los id de los pedidos si existe
			
			try {
				con = ConexionBDD.obtenerConexion();
				
				
				// Verifico que exista el proveedor introducido y si existe extraigo el id con sus datos: 
				psBuscarProveedor= con.prepareStatement( "select id_proveedor, codigo_documento, nombre from proveedores where id_proveedor = ?"
				);
				
				psBuscarProveedor.setString(1, idProveedorConsulta);
				rsProveedor = psBuscarProveedor.executeQuery();

				if (!rsProveedor.next()) {
					throw new KrakeDevException("No existe proveedor con el id: " + idProveedorConsulta);
				}
				
				// Obtengo el proveedor y datos: 
		        TipoDocumento tipoDocumentoProveedor = new TipoDocumento();
		        tipoDocumentoProveedor.setCodigoDocumento(rsProveedor.getString("codigo_documento"));

		        Proveedor proveedorBase = new Proveedor();
		        proveedorBase.setIdProveedor(rsProveedor.getString("id_proveedor"));
		        proveedorBase.setNombre(rsProveedor.getString("nombre"));
		        proveedorBase.setTipoDocumento(tipoDocumentoProveedor);

		        rsProveedor.close();
		        psBuscarProveedor.close();
		        
		        // Busco con el id de proveedor en los pedidos: 
		        psCabeceras = con.prepareStatement(
		        	    "select distinct cp.id_cabecera_pedidos, cp.fecha, cp.codigo_estado "
		        ...
		        psCabeceras.setString(1, idProveedorConsulta);
		        rsCabeceras = psCabeceras.executeQuery();

		        while (rsCabeceras.next()) {
		        Pedidos pedido = new Pedidos();

		        int idCabecera = rsCabeceras.getInt("id_cabecera_pedidos");
		        pedido.setIdCabeceraPedidos(idCabecera);
		        pedido.setFecha(rsCabeceras.getDate("fecha").toString());

		        EstadoPedido estado = new EstadoPedido();
		        estado.setCodigoEstado(rsCabeceras.getString("codigo_estado"));
		        pedido.setCodigoEstado(estado);

		        pedido.setIdProveedor(proveedorBase);
		        pedido.setDetalles(new ArrayList<DetallePedido>());
		                
		        // Con el id del pedido busco detalles de ese pedido: 
		        	psDetalles = con.prepareStatement(
		                	    "select dp.id_pedidos, dp.codigo_prod, pro.nombre as nombre_producto, "
		                	    + "cat.nombre as nombre_categoria, dp.cantidad_recibida, dp.subtotal "
		                	    ...
		                	    + "where dp.id_cabecera_pedidos = ? "
		                	    + "order by dp.codigo_prod"
		        			);
		                
		        	psDetalles.setInt(1, idCabecera);
		        	rsDetalles = psDetalles.executeQuery();

		        	while (rsDetalles.next()) {
		        		Categoria categoria = new Categoria();
		        		categoria.setNombre(rsDetalles.getString("nombre_categoria"));

		        		Producto producto = new Producto();
		        		producto.setCodigoProd(rsDetalles.getInt("codigo_prod"));
		        		producto.setNombre(rsDetalles.getString("nombre_producto"));
		        		producto.setCodigoCategoria(categoria);

		        		DetallePedido detalle = new DetallePedido();
		        		detalle.setIdPedidos(rsDetalles.getInt("id_pedidos"));
		        		detalle.setCodigoProd(producto);
		        		detalle.setCantidadRecibida(rsDetalles.getInt("cantidad_recibida"));
		        ...
		        psCabeceras.close();
		            ...
		            // Si la lista esta vacia, existe el proveedor pero no tiene pedidos: 
		        if (pedidos.isEmpty()) {
		                throw new KrakeDevException("El proveedor " + idProveedorConsulta + "no registra pedidos");
		        }  
		                				 
			} catch (SQLException e) {
				e.printStackTrace();
				throw new KrakeDevException("Error SQL: " + e.getMessage()); // Error en la sql 
			} finally {
				try {
					if (rsProveedor != null) rsProveedor.close();
		            if (rsCabeceras != null) rsCabeceras.close();
		            if (rsDetalles != null) rsDetalles.close();
		            if (psBuscarProveedor != null) psBuscarProveedor.close();
		            if (psCabeceras != null) psCabeceras.close();
	...
	public void recibirPedido(Pedidos pedido) throws KrakeDevException {
		...
		PreparedStatement psInsertarHistorial = null; // Inserto historial de stock
		ResultSet rs = null;
		...
		try {
			...
			for (int i = 0; i < pedido.getDetalles().size(); i++) {
				...
				psInsertarHistorial = con.prepareStatement(
					...
					psInsertarHistorial.setInt(3, codigoProducto);
					psInsertarHistorial.setInt(4, detalle.getCantidadRecibida());

					int filasHistorial = psInsertarHistorial.executeUpdate();

					if (filasHistorial == 0) {
						throw new KrakeDevException("No se pudo insertar el historial de stock del producto " + codigoProducto);
					}
