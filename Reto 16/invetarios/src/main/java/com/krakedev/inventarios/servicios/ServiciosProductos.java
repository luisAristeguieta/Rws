...
public class ServiciosProductos {
	...
	public Response agregarProducto(Producto producto) {
		...
		try {
			...
			return Response.ok("Producto Agregado Correctamente").build(); // Marcaria 200 en postman
		} catch (KrakeDevException e) {
			e.printStackTrace();
	...
	@GET
    @Path("buscarID/{codigo}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response buscarProducto(@PathParam("codigo") int codigoProducto) {
		ProductoBDD prodBDD = new ProductoBDD();

        try {
            Producto producto = prodBDD.buscarProducto(codigoProducto);
